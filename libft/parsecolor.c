...
/*   Updated: 2018/01/18 13:13:19 by ygarrot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		parse_colors(char *str, t_arg *tempm)
{
	...
	att = ft_strsplit("reset,bold,low,italic,blink,reverse,conc", ',');
	colors = ft_strsplit("black,red,green,yellow,blue,magenta,cyan,white", ',');
	index = 1;
	temp = ft_strisin_tab(&str[index], att, 0);
	tempm->att = (temp >= 0) ? temp : -1;
	index += (temp >= 0) ? ft_strlen(att[temp]) : 0;
	temp = ft_strisin_tab(&str[index], colors, 0);
	tempm->fg = (temp >= 0) ? temp : -1;
	index += (temp >= 0) ? ft_strlen(colors[temp]) : 0;
	temp = ft_strisin_tab(&str[index], colors, 0);
	tempm->bg = (temp >= 0) ? temp : -1;
	index += (temp >= 0) ? ft_strlen(colors[temp]) : 0;
	ft_free_dblechar_tab(att);
	ft_free_dblechar_tab(colors);
